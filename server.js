...
// ✅ /quentes_ai retorna 2 jogos em dias úteis e 3 no fim de semana, com horários diferentes e intervalo mínimo
...
// ✅ NOVO: TOP DO DIA: 2 em dias úteis / 3 no fim de semana, melhores por score com horários diferentes e intervalo mínimo
// ✅ NOVO: PRIORIDADE FORTE para ligas maiores e times agressivos em escanteios
// ✅ NOVO: funil extra home_response (sem alterar sua lógica central)
// ✅ NOVO: POWER PRE-GAME +9.5 (20 times): líder/vice fora, 3º/4º fora filtrado, clássico, elite vs elite, 1º x 17º, 5º x 12º, 9º x 16º
// ✅ NOVO: BLOQUEIO DURO playoff/mata-mata + ritmo baixo também em FULL, SEMI e LITE
...
// 1) arquivos dentro da pasta /public (recomendado no VS Code)
// 2) arquivos soltos na raiz do GitHub (caso o upload tenha sido feito assim)
...
// ✅ Regra operacional: horários diferentes e com intervalo mínimo entre partidas,
//    para dar tempo de acompanhar o jogo e fazer uma possível segunda entrada.
// ✅ Preferência padrão: 60 minutos. Se não fechar a quantidade, tenta 30 minutos.
const DAILY_TOP_GAMES_LIMIT = Number(process.env.DAILY_TOP_GAMES_LIMIT || 2);
const WEEKEND_TOP_GAMES_LIMIT = Number(process.env.WEEKEND_TOP_GAMES_LIMIT || 3);
const MIN_GAME_GAP_MINUTES = Number(process.env.MIN_GAME_GAP_MINUTES || 60);
const FALLBACK_MIN_GAME_GAP_MINUTES = Number(process.env.FALLBACK_MIN_GAME_GAP_MINUTES || 30);

// 🔒 Blindagem principal dos cards do site:
// jogos que visualmente ficariam com card vermelho/perigoso NÃO entram no Top do dia.
// Mantém o modo clássico de horários diferentes, mas agora troca jogo perigoso pelo próximo melhor seguro.
const BLOCK_DANGEROUS_RED_CARDS = String(process.env.BLOCK_DANGEROUS_RED_CARDS || "1") === "1";
const RED_CARD_MIN_PROJ = Number(process.env.RED_CARD_MIN_PROJ || 10.6);
const RED_CARD_MIN_PROB = Number(process.env.RED_CARD_MIN_PROB || 68);
const RED_CARD_MIN_AI_SCORE = Number(process.env.RED_CARD_MIN_AI_SCORE || 70);
...
function isBlockedAwayFavoriteForSelection(x) {
  // Blindagem final: favorito fora continua perigoso, mas agora aceita somente
  // os casos aprovados pelo POWER PRE-GAME: 3º/4º fora com filtros fortes
  // ou substituto elite real.
  if (!isAwayFavoriteStrict(x?.odds, x?.pos_home, x?.pos_away)) return false;
  if (isEliteAwayReplacementGame(x)) return false;
  if (Array.isArray(x?.flags) && x.flags.includes("away_top34_filtered_ok")) return false;
  return true;
}
...
const LEAGUE_META = {
  // 🔥 Ligas maiores e naturalmente melhores para cantos recebem peso alto
  152: { name: "Premier League", baseCorners: 11.0, importance: 99 },
  175: { name: "Bundesliga", baseCorners: 11.1, importance: 98 },
  244: { name: "Eredivisie", baseCorners: 10.9, importance: 96 },
  168: { name: "Ligue 1", baseCorners: 10.6, importance: 95 },
  207: { name: "Serie A", baseCorners: 10.3, importance: 94 },
  302: { name: "La Liga", baseCorners: 10.5, importance: 93 },
  266: { name: "Primeira Liga", baseCorners: 10.4, importance: 92 },
  63:  { name: "Belgium First Division A", baseCorners: 10.5, importance: 91 },

  // Boas ligas, mas abaixo do bloco principal
  56:  { name: "Austria Bundesliga", baseCorners: 10.1, importance: 84 },
  322: { name: "Süper Lig", baseCorners: 10.1, importance: 84 },
  279: { name: "Premiership", baseCorners: 10.1, importance: 83 },
  135: { name: "Denmark Superliga", baseCorners: 10.2, importance: 82 },
  253: { name: "Eliteserien", baseCorners: 10.2, importance: 80 },
  307: { name: "Allsvenskan", baseCorners: 10.0, importance: 78 },

  // Ligas secundárias: só sobem se o jogo for muito forte em score/pressão/cantos
  308: { name: "Super League (Suíça)", baseCorners: 9.8, importance: 76 },
  259: { name: "Ekstraklasa", baseCorners: 9.8, importance: 70 },
  134: { name: "Czech Liga", baseCorners: 9.7, importance: 69 },
  178: { name: "Super League 1", baseCorners: 9.6, importance: 68 },
  124: { name: "HNL", baseCorners: 9.5, importance: 67 },
  272: { name: "Liga I", baseCorners: 9.5, importance: 66 },

  99:  { name: "Brasileirão Série A", baseCorners: 10.2, importance: 89 },
  18:  { name: "Libertadores", baseCorners: 9.8, importance: 86 },

  3:   { name: "Champions League", baseCorners: 10.4, importance: 90 },
  4:   { name: "Europa League", baseCorners: 10.1, importance: 88 },
  683: { name: "Conference League", baseCorners: 10.0, importance: 78 },
};
...
/* =========================================================
   🔥 PRIORIDADE PREMIUM — LIGAS MAIORES + TIMES AGRESSIVOS EM CANTOS
   ========================================================= */
const PREMIUM_CORNER_LEAGUE_IDS = new Set([152,175,244,168,207,302,266,63]);
const GOOD_CORNER_LEAGUE_IDS = new Set([56,322,279,135,253,307,99,18,3,4]);
const SECONDARY_RISK_LEAGUE_IDS = new Set([259,134,178,124,272,308,683]);

const PREMIUM_LEAGUE_NAME_PATTERNS = [
  "premier league", "bundesliga", "eredivisie", "ligue 1", "serie a", "la liga",
  "primeira liga", "liga portugal", "belgium", "jupiler", "championship"
];

const SECONDARY_LEAGUE_NAME_PATTERNS = [
  "ekstraklasa", "czech", "liga i", "hnl", "super league 1", "romania", "poland", "croatia"
];

const AGGRESSIVE_CORNER_TEAMS = new Set([
  // Inglaterra
  "Liverpool", "Arsenal", "Tottenham", "Manchester City", "Manchester United", "Newcastle", "Brighton", "Chelsea", "Aston Villa", "West Ham",
  // Alemanha
  "Borussia Dortmund", "Dortmund", "Bayern Munich", "Bayer Leverkusen", "RB Leipzig", "Eintracht Frankfurt", "Stuttgart", "Hoffenheim", "Wolfsburg",
  // França
  "Lens", "Lens FC", "Racing Club de Lens", "Marseille", "Lyon", "PSG", "Monaco", "Rennes", "Lille", "Nice",
  // Holanda
  "Ajax", "PSV", "Feyenoord", "AZ Alkmaar", "Twente", "Utrecht",
  // Itália
  "Inter", "AC Milan", "Atalanta", "Napoli", "Roma", "Lazio", "Fiorentina", "Bologna", "Juventus",
  // Espanha
  "Real Madrid", "Barcelona", "Atletico Madrid", "Girona", "Real Sociedad", "Athletic Bilbao", "Villarreal", "Sevilla", "Real Betis",
  // Portugal / Bélgica
  "Porto", "Benfica", "Sporting", "Braga", "Club Brugge", "Genk", "Gent", "Anderlecht", "Union Saint-Gilloise",
  // Brasil
  "Flamengo", "Palmeiras", "Atlético Mineiro", "Fluminense", "Grêmio", "Internacional", "Botafogo", "São Paulo", "Athletico Paranaense"
]);
...
function leaguePriorityBonus(x){
  const id = Number(x?.league_id ?? x?.league?.id ?? NaN);
  const name = normLooseText(x?.liga ?? x?.league?.name ?? "");

  if (PREMIUM_CORNER_LEAGUE_IDS.has(id)) return 16;
  if (GOOD_CORNER_LEAGUE_IDS.has(id)) return 7;
  if (SECONDARY_RISK_LEAGUE_IDS.has(id)) return -14;

  if (PREMIUM_LEAGUE_NAME_PATTERNS.some(p => name.includes(normLooseText(p)))) return 14;
  if (SECONDARY_LEAGUE_NAME_PATTERNS.some(p => name.includes(normLooseText(p)))) return -12;

  return 0;
}

function isAggressiveCornerTeamName(teamName){
  const key = normLooseText(teamName);
  if (!key) return false;
  for (const t of AGGRESSIVE_CORNER_TEAMS) {
    const tk = normLooseText(t);
    if (key === tk || key.includes(tk) || tk.includes(key)) return true;
  }
  return false;
}

function aggressiveCornerTeamsBonus(x){
  const home = isAggressiveCornerTeamName(x?.casa);
  const away = isAggressiveCornerTeamName(x?.fora);
  if (home && away) return 18;
  if (home || away) return 10;
  return 0;
}

function widePressureBonus(x){
  let bonus = 0;
  if (String(x?.perfil_laterais || "") === "LATERAIS_FORTES") bonus += 8;
  if (String(x?.perfil_laterais || "") === "EQUILIBRADO") bonus += 2;

  const press = Number(x?.real?.pressureHits ?? 0);
  if (Number.isFinite(press) && press >= 4) bonus += 6;
  else if (Number.isFinite(press) && press >= 3) bonus += 3;

  const homeCorners = Number(x?.real?.homeRecent?.cornersForAvg ?? NaN);
  const awayCorners = Number(x?.real?.awayRecent?.cornersForAvg ?? NaN);
  if (Number.isFinite(homeCorners) && homeCorners >= 5.8) bonus += 4;
  if (Number.isFinite(awayCorners) && awayCorners >= 5.4) bonus += 4;

  return bonus;
}

function premiumCornerRankingScore(x){
  const ai = Number.isFinite(x?.ai_score) ? x.ai_score : aiScoreFromMatch(x);
  const p = Number(x?.over95_prob_adj ?? x?.over95_prob ?? 0);
  const proj = Number(x?.proj_cantos ?? 0);
  const scoreBase = Number(x?.score_adj ?? x?.score ?? 0);
...
// ✅ FAVORITO FORA SUPER FORTE (liberação controlada)
function isStrongAwayFavoriteRelease({
  oddsInfo,
  posHome,
  posAway,
  proj_cantos,
  ...
  if (favSide !== "AWAY") return false;

  if (!Number.isFinite(proj_cantos) || proj_cantos < 11.2) return false;

  if ((awayRecent?.cornersForAvg ?? 0) < 5.2) return false;

  if ((homeRecent?.cornersAgainstAvg ?? 0) < 4.8) return false;

  if ((homeRecent?.cornersForAvg ?? 0) < 4.2) return false;

  if (perfil_laterais === "TENDENCIA_CENTRAL") return false;

  if (!Number.isFinite(pressureHits) || pressureHits < 3) return false;

  // visitante muito dominante na tabela
  if (
    Number.isFinite(posAway) &&
    Number.isFinite(posHome)
  ){
    const diff = posHome - posAway;

    // visitante melhor colocado
...
function antiRedGlobalCheck({
  ...
  if (
    favSide === "AWAY" &&
    !isStrongAwayFavoriteRelease({
      oddsInfo,
      posHome,
      posAway,
      proj_cantos: recentCombinedAvg,
      homeRecent,
      awayRecent,
      perfil_laterais,
      pressureHits
...
/* =========================================================
   🔒 BLOQUEIO: PLAYOFF + RITMO BAIXO
   ========================================================= */
function getEventContextText(eventData = {}, leagueName = ""){
  return [
    leagueName,
    eventData?.league_name,
    eventData?.match_round,
    eventData?.round,
    eventData?.stage,
    eventData?.match_stage,
    eventData?.match_type,
    eventData?.league_round,
    eventData?.event_round,
    eventData?.match_name
  ]
    .filter(Boolean)
    .join(" ")
    .toLowerCase()
...
function isPlayoffOrKnockoutContext(eventData = {}, leagueName = ""){
  const txt = getEventContextText(eventData, leagueName);

  return (
    txt.includes("playoff") ||
    txt.includes("play-offs") ||
    txt.includes("play offs") ||
    txt.includes("play off") ||
    txt.includes("promotion") ||
    txt.includes("relegation") ||
    txt.includes("semi-final") ||
    txt.includes("semi final") ||
    txt.includes("semifinal") ||
    txt.includes("quarter-final") ||
    txt.includes("quarter final") ||
    txt.includes("final") ||
    txt.includes("knockout") ||
    txt.includes("eliminat") ||
    txt.includes("round of") ||
    txt.includes("final stage")
  );
}

function lowRhythmPlayoffBlock({
  eventData,
  leagueName = "",
  pressureHits,
  perfil_laterais,
  ...
  // Sem base completa em jogo de playoff já é sinal de risco no pré-jogo
  if (mode === "semi" || mode === "lite") lowSignals++;

  // pressão baixa
  if (Number.isFinite(Number(pressureHits)) && Number(pressureHits) <= 2) lowSignals++;

  // sem pressão medida em semi/lite também é risco
  if (!Number.isFinite(Number(pressureHits)) && (mode === "semi" || mode === "lite")) lowSignals++;

  // tendência central ou jogo apenas equilibrado em playoff
  if (perfil_laterais === "TENDENCIA_CENTRAL") lowSignals += 2;
  else if (perfil_laterais === "EQUILIBRADO") lowSignals++;

  // média recente ruim
  if (Number.isFinite(Number(recentCombinedAvg)) && Number(recentCombinedAvg) < 9.8) lowSignals++;

  // projeção ruim ou apenas mediana para linha +9.5
  if (Number.isFinite(Number(proj_cantos)) && Number(proj_cantos) < 10.6) lowSignals++;

  const block = lowSignals >= 2;

  return {
    block,
    reason: block ? "playoff_low_rhythm_block" : null,
    lowSignals,
    isPlayoff: true
  };
}
...
/* =========================================================
   🔥 POWER PRE-GAME +9.5 — MODELO 20 TIMES
   - 1º/2º fora: bloqueio padrão
   - 3º/4º fora: só passa com filtro forte
   - 1º x 17º / 2º x 18º: risco de matar cedo
   - 5º x 12º e 9º x 16º: cenários positivos se os números confirmarem
   - Elite vs elite: bloqueio de segurança
   ========================================================= */
const POWER_PRE_GAME_95 = {
  ENABLE: true,
  MIN_PROJ_SAFE: 10.5,
  MIN_PROJ_GOLD: 11.0,
  MIN_PROJ_SUPER_RELEASE: 11.5,
  MIN_HOME_CORNERS_FOR: 5.0,
  MIN_AWAY_CORNERS_FOR: 3.5,
  MIN_AWAY_CORNERS_FOR_TOP34: 4.0,
  MIN_AWAY_ELITE_CORNERS_FOR: 5.0,
  MIN_HOME_CONCEDES_FOR_AWAY_ELITE: 4.8,
  LEADER_AWAY_MAX_POS: 2,
  TOP34_AWAY_MIN_POS: 3,
  TOP34_AWAY_MAX_POS: 4,
  VERY_WEAK_FROM_POS: 17,
  ELITE_MAX_POS: 4,
  HIGH_ELITE_MAX_POS: 7,
  BONUS_IDEAL_4_7_VS_10_14: 14,
  BONUS_8_12_VS_14_17: 8,
  BONUS_TOP34_AWAY_OK: 5,
  PENALTY_HIGH_ELITE: 12,
};

function bothPositionsKnown(posHome, posAway){
  return Number.isFinite(posHome) && Number.isFinite(posAway);
}

function isSuperCornerRelease({ proj_cantos, homeRecent, awayRecent, pressureHits }){
  return (
    Number.isFinite(proj_cantos) && proj_cantos >= POWER_PRE_GAME_95.MIN_PROJ_SUPER_RELEASE &&
    Number.isFinite(awayRecent?.cornersForAvg) && awayRecent.cornersForAvg >= POWER_PRE_GAME_95.MIN_AWAY_ELITE_CORNERS_FOR &&
    Number.isFinite(homeRecent?.cornersAgainstAvg) && homeRecent.cornersAgainstAvg >= POWER_PRE_GAME_95.MIN_HOME_CONCEDES_FOR_AWAY_ELITE &&
    Number.isFinite(homeRecent?.cornersForAvg) && homeRecent.cornersForAvg >= POWER_PRE_GAME_95.MIN_AWAY_CORNERS_FOR &&
    Number.isFinite(pressureHits) && pressureHits >= 3
  );
}

function classifyPower95(scoreAdj, aiScore, proj, flags = []){
  const s = Number.isFinite(aiScore) ? aiScore : Math.round((Number(scoreAdj || 0) / 150) * 100);
  if (flags.some(f => String(f).startsWith("risk_") || String(f).startsWith("warn_"))) {
    if (s >= 82 && Number(proj) >= 10.8) return "BOM_COM_ALERTA";
  }
  if (s >= 90 && Number(proj) >= 11) return "OURO";
  if (s >= 80 && Number(proj) >= 10.7) return "MUITO_BOM";
  if (s >= 70 && Number(proj) >= 10.3) return "BOM";
  if (s >= 60) return "ARRISCADO";
  return "FORA";
}

function powerPreGame95Guard({
  posHome,
  posAway,
  favSide,
  proj_cantos,
  ...
  if (!POWER_PRE_GAME_95.ENABLE) {
    return { block:false, scoreBonus:0, scorePenalty:0, flags:[], reason:null };
  }

  const flags = [];
  let scoreBonus = 0;
  let scorePenalty = 0;
  const known = bothPositionsKnown(posHome, posAway);
  const superRelease = isSuperCornerRelease({ proj_cantos, homeRecent, awayRecent, pressureHits });

  if (isClassic && !superRelease) {
    return { block:true, scoreBonus, scorePenalty, flags:["classic_block"], reason:"Clássico/rivalidade forte: bloqueio padrão para pré-jogo +9.5." };
  }
  if (isClassic && superRelease) {
    flags.push("classic_super_release_risk");
    scorePenalty += 18;
  }

  if (isKnockout) {
    flags.push("risk_knockout_context");
    scorePenalty += 10;
  }

  if (known) {
    if (posHome <= POWER_PRE_GAME_95.ELITE_MAX_POS && posAway <= POWER_PRE_GAME_95.ELITE_MAX_POS && !superRelease) {
      return { block:true, scoreBonus, scorePenalty, flags:["elite_vs_elite_block"], reason:"Elite vs elite: jogo tende a ficar estudado/travado." };
    }

    if (posHome <= POWER_PRE_GAME_95.HIGH_ELITE_MAX_POS && posAway <= POWER_PRE_GAME_95.HIGH_ELITE_MAX_POS) {
      flags.push("risk_high_table_balance");
      scorePenalty += POWER_PRE_GAME_95.PENALTY_HIGH_ELITE;
    }

    if (favSide === "AWAY" && posAway <= POWER_PRE_GAME_95.LEADER_AWAY_MAX_POS && !superRelease) {
      return { block:true, scoreBonus, scorePenalty, flags:["leader_or_vice_away_block"], reason:"1º/2º fora: risco alto de controle, gol cedo e queda de ritmo." };
    }
    if (favSide === "AWAY" && posAway <= POWER_PRE_GAME_95.LEADER_AWAY_MAX_POS && superRelease) {
      flags.push("leader_or_vice_away_super_release");
      scorePenalty += 14;
    }

    if (favSide === "AWAY" && posAway >= POWER_PRE_GAME_95.TOP34_AWAY_MIN_POS && posAway <= POWER_PRE_GAME_95.TOP34_AWAY_MAX_POS) {
      if (posHome >= POWER_PRE_GAME_95.VERY_WEAK_FROM_POS && !superRelease) {
        return { block:true, scoreBonus, scorePenalty, flags:["top34_away_vs_very_weak_block"], reason:"3º/4º fora contra 17º+ pode matar o jogo cedo." };
      }
      if (Number.isFinite(awayRecent?.cornersForAvg) && awayRecent.cornersForAvg < POWER_PRE_GAME_95.MIN_AWAY_CORNERS_FOR_TOP34) {
        return { block:true, scoreBonus, scorePenalty, flags:["top34_away_low_corners_block"], reason:"3º/4º fora com baixa produção de cantos fora." };
      }
      if (Number.isFinite(proj_cantos) && proj_cantos < POWER_PRE_GAME_95.MIN_PROJ_SAFE) {
        return { block:true, scoreBonus, scorePenalty, flags:["top34_away_low_projection_block"], reason:"3º/4º fora sem projeção mínima para +9.5." };
      }
      flags.push("away_top34_filtered_ok");
      scoreBonus += POWER_PRE_GAME_95.BONUS_TOP34_AWAY_OK;
    }

    if (posHome <= POWER_PRE_GAME_95.LEADER_AWAY_MAX_POS && posAway >= POWER_PRE_GAME_95.VERY_WEAK_FROM_POS && !superRelease) {
      return { block:true, scoreBonus, scorePenalty, flags:["top2_home_vs_17plus_block"], reason:"1º/2º vs 17º+ costuma ter risco de gol cedo e queda de pressão." };
    }

    if (posHome >= 3 && posHome <= 4 && posAway >= POWER_PRE_GAME_95.VERY_WEAK_FROM_POS && !superRelease) {
      return { block:true, scoreBonus, scorePenalty, flags:["top34_home_vs_17plus_block"], reason:"3º/4º vs 17º+ também pode virar jogo morto cedo." };
    }

    if (posHome >= 4 && posHome <= 7 && posAway >= 10 && posAway <= 14) {
      flags.push("ideal_4_7_vs_10_14");
      scoreBonus += POWER_PRE_GAME_95.BONUS_IDEAL_4_7_VS_10_14;
    }

    if (posHome >= 8 && posHome <= 12 && posAway >= 14 && posAway <= 17) {
      flags.push("good_8_12_vs_14_17");
      scoreBonus += POWER_PRE_GAME_95.BONUS_8_12_VS_14_17;
    }
  }

  if (Number.isFinite(proj_cantos) && proj_cantos < 10.0) {
    return { block:true, scoreBonus, scorePenalty, flags:[...flags, "projection_under_10_block"], reason:"Projeção abaixo de 10: ruim para pré-jogo +9.5." };
  }

  if (Number.isFinite(homeRecent?.cornersForAvg) && homeRecent.cornersForAvg < POWER_PRE_GAME_95.MIN_AWAY_CORNERS_FOR) {
    flags.push("warn_home_low_corner_creation");
    scorePenalty += 8;
  }

  if (Number.isFinite(awayRecent?.cornersForAvg) && awayRecent.cornersForAvg < POWER_PRE_GAME_95.MIN_AWAY_CORNERS_FOR) {
    flags.push("warn_away_low_corner_creation");
    scorePenalty += 8;
  }

  if (perfil_laterais === "TENDENCIA_CENTRAL") {
    flags.push("risk_central_style_power95");
    scorePenalty += 8;
  }

  return { block:false, scoreBonus, scorePenalty, flags, reason:null };
}
...
function dailyTopLimitForDate(dateStr){
  return isWeekendDate(dateStr) ? WEEKEND_TOP_GAMES_LIMIT : DAILY_TOP_GAMES_LIMIT;
}
...
/* =========================================================
   ✅ MERCADOS EXTRAS — GOLS + LINHAS DE CANTOS
   IMPORTANTE: não altera o motor/ranking principal de +9.5 cantos.
   Apenas acrescenta o objeto `markets` no retorno dos jogos.
   ========================================================= */
...
function getMatchScorePair(g){
  const hs = scoreValue(
    g?.match_hometeam_score ??
    g?.match_hometeam_ft_score ??
    g?.home_score ??
    g?.score_home ??
    g?.goals_home
  );
  ...
  const as = scoreValue(
    g?.match_awayteam_score ??
    g?.match_awayteam_ft_score ??
    g?.away_score ??
    g?.score_away ??
    g?.goals_away
  );
  ...
  return { home: hs, away: as, total: hs + as };
}

function teamGoalsProfileFromH2H(teamName, h2hBlock, which, lastN = 5){
  const list = Array.isArray(h2hBlock?.[which]) ? h2hBlock[which] : [];
  const teamKey = normTeamKey(teamName);
  if (!teamKey || !list.length) return null;
  ...
  for (const g of list.slice(0, lastN)){
    const score = getMatchScorePair(g);
    if (!score) continue;

    const homeName = normTeamKey(
    ...
    if (homeName) isHome = homeName === teamKey || homeName.includes(teamKey) || teamKey.includes(homeName);
    if (awayName) isAway = awayName === teamKey || awayName.includes(teamKey) || teamKey.includes(awayName);
    ...
    if (gf >= 1) scoredGames++;
    if (ga >= 1) concededGames++;
    if (score.total >= 2) over15Games++;
    if (score.total >= 3) over25Games++;
    if (score.total >= 4) over35Games++;
  ...
  return {
    ...
    totalGoalsAvg: totalGoals / games,
    scoredRate: (scoredGames / games) * 100,
    concededRate: (concededGames / games) * 100,
    over15Rate: (over15Games / games) * 100,
    over25Rate: (over25Games / games) * 100,
    over35Rate: (over35Games / games) * 100,
  };
}

function estimateGoalMarkets({
  game,
  h2h = null,
  lastN = 5
} = {}){
  ...
  if (homeProfile && awayProfile){
    homeExpected = ((homeProfile.goalsForAvg || 0) + (awayProfile.goalsAgainstAvg || 0)) / 2;
    awayExpected = ((awayProfile.goalsForAvg || 0) + (homeProfile.goalsAgainstAvg || 0)) / 2;
    totalExpected = homeExpected + awayExpected;
  } else {
    // Fallback leve para jogos sem histórico de gols.
    // Não interfere no motor de cantos; serve apenas para a aba de filtros não ficar cega.
    const leagueImportance = Number(game?.score ?? game?.score_adj ?? 0);
    const posHome = Number(game?.pos_home ?? NaN);
    const posAway = Number(game?.pos_away ?? NaN);
    const diff = (Number.isFinite(posHome) && Number.isFinite(posAway)) ? Math.abs(posHome - posAway) : 7;
    const isBig = isBigTeam(game?.casa) || isBigTeam(game?.fora);

    totalExpected = 2.05;
    if (leagueImportance >= 95) totalExpected += 0.25;
    if (isBig) totalExpected += 0.18;
    if (diff <= 6) totalExpected += 0.12;
    if (diff >= 12) totalExpected -= 0.10;

    homeExpected = totalExpected / 2;
    awayExpected = totalExpected / 2;
  }

  const scoredAvg =
    homeProfile && awayProfile
      ? (homeProfile.scoredRate + awayProfile.scoredRate + homeProfile.concededRate + awayProfile.concededRate) / 4
      : 52;

  const over15Hist =
    homeProfile && awayProfile
      ? (homeProfile.over15Rate + awayProfile.over15Rate) / 2
      : 55;

  const over25Hist =
    homeProfile && awayProfile
      ? (homeProfile.over25Rate + awayProfile.over25Rate) / 2
      : 45;

  const over35Hist =
    homeProfile && awayProfile
      ? (homeProfile.over35Rate + awayProfile.over35Rate) / 2
      : 28;

  const minTeamExpected = Math.min(Number(homeExpected || 0), Number(awayExpected || 0));

  const bttsProb = clamp(
    Math.round(28 + minTeamExpected * 24 + scoredAvg * 0.28),
    5,
    90
  );

  const over15Prob = clamp(
    Math.round(38 + (Number(totalExpected || 0) - 1.5) * 22 + over15Hist * 0.30),
    5,
    95
  );

  const over25Prob = clamp(
    Math.round(28 + (Number(totalExpected || 0) - 2.5) * 23 + over25Hist * 0.32),
    5,
    90
  );

  const over35Prob = clamp(
    Math.round(18 + (Number(totalExpected || 0) - 3.5) * 20 + over35Hist * 0.28),
    3,
    82
  );
  ...
  return {
    btts: bttsProb >= 55,
    over15: over15Prob >= 62,
    over25: over25Prob >= 55,
    over35: over35Prob >= 48,
    prob: {
      btts: bttsProb,
      over15: over15Prob,
      over25: over25Prob,
    ...
    expected: {
      home: Number.isFinite(homeExpected) ? Number(homeExpected.toFixed(2)) : null,
      away: Number.isFinite(awayExpected) ? Number(awayExpected.toFixed(2)) : null,
      total: Number.isFinite(totalExpected) ? Number(totalExpected.toFixed(2)) : null,
    },
    sources: {
      goals_h2h: !!(homeProfile && awayProfile)
    }
  };
}

function estimateCornerMarkets(game){
  const proj = Number(game?.proj_cantos ?? 0);
  const p95 = Number(game?.over95_prob_adj ?? game?.over95_prob ?? probFromProjection(proj));

  // Estimativas derivadas da sua própria projeção de cantos.
  // Não mexe na seleção +9.5; só cria as linhas extras.
  const p105 = clamp(Math.round(p95 - 8 + Math.max(0, proj - 10.5) * 4), 3, 92);
  const p115 = clamp(Math.round(p95 - 16 + Math.max(0, proj - 11.2) * 5), 2, 88);
...
function withExtraMarkets(game, context = {}){
  if (!game) return game;
  const current = game.markets && typeof game.markets === "object" ? game.markets : {};
  const extra = buildExtraMarkets(game, context);
...
function projCornersHeuristic(baseCorners, isBigMatch, posHome, posAway) {
  ...
  if (posHome && posAway) {
    ...
    if (posHome <= 4 || posAway <= 4) proj += 0.3;
  }
...
function aiScoreFromMatch(x){
  ...
  // 🔥 Ajuste premium: sobe ligas grandes e times agressivos em cantos;
  // derruba ligas secundárias quando o jogo não for muito forte.
  s += leaguePriorityBonus(x) * 0.45;
  s += aggressiveCornerTeamsBonus(x) * 0.55;
  s += widePressureBonus(x) * 0.45;
...
function liteFromEvent(e, league, posHome = null, posAway = null, lite_reason = "no_base"){
  ...
  // 🔒 BLOQUEIO DURO NO LITE: playoff/mata-mata com ritmo baixo não vira card reserva
  const playoffLowRhythmLite = lowRhythmPlayoffBlock({
    eventData: e,
    leagueName: league.name,
    pressureHits: null,
    perfil_laterais,
    recentCombinedAvg: null,
    proj_cantos,
    mode: "lite"
  });

  if (playoffLowRhythmLite.block) return null;
  ...
  const liteWithScore = { ...lite, ai_score: aiScoreFromMatch(lite) };
  return withExtraMarkets(liteWithScore);
}
...
// ✅ TOP DO DIA — 2 em dias úteis / 3 no fim de semana, com horários diferentes.
// Regra operacional:
// 1) ordena por qualidade/score;
// 2) pega os melhores que tenham horário diferente;
// 3) exige intervalo preferencial de 60 min entre jogos;
// 4) se não fechar a quantidade, tenta intervalo mínimo de 30 min;
// 5) se ainda não fechar, devolve apenas os melhores que respeitam a regra.
function isDangerousRedCardGame(x){
  if (!BLOCK_DANGEROUS_RED_CARDS) return false;
  if (!x) return true;
  ...
  const ai = Number.isFinite(x?.ai_score) ? x.ai_score : aiScoreFromMatch(x);
  const perfil = String(x?.perfil_laterais ?? "");
  const nivel = String(x?.nivel_power95 ?? x?.nivel ?? x?.classificacao ?? x?.power95_class ?? "").toUpperCase();
  ...
  const txt = [
    ...
    x?.nivel,
    x?.classificacao,
    ...(Array.isArray(x?.ai_human?.chips) ? x.ai_human.chips : []),
    ...(Array.isArray(x?.chips) ? x.chips : []),
    ...flags
  ].filter(Boolean).join(" ").toLowerCase();

  // 🚫 Card vermelho/perigoso: não entra no Top do dia.
  if (risk === "alta") return true;
  if (isBlockedAwayFavoriteForSelection(x)) return true;
  if (perfil === "TENDENCIA_CENTRAL") return true;
  if (p > 0 && p < RED_CARD_MIN_PROB) return true;
  if (proj > 0 && proj < RED_CARD_MIN_PROJ) return true;
  if (ai < RED_CARD_MIN_AI_SCORE) return true;
  if (nivel === "ARRISCADO" || nivel === "FORA") return true;

  if (txt.includes("risco ia")) return true;
  if (txt.includes("risco alto")) return true;
  if (txt.includes("atenção ia")) return true;
  if (txt.includes("ritmo baixo")) return true;
  if (txt.includes("cenário mais traiçoeiro")) return true;
  if (txt.includes("cenario mais traicoeiro")) return true;
  if (txt.includes("tendência central")) return true;
  if (txt.includes("tendencia central")) return true;

  if (flags.some(f =>
    f.startsWith("red_") ||
    f.startsWith("risk_") ||
    f.includes("low_corners") ||
    f.includes("low_projection") ||
    f.includes("low_expected") ||
    f.includes("bad_history") ||
    f.includes("central_style") ||
    f.includes("one_sided") ||
    f.includes("no_pillars") ||
    f.includes("playoff_low_rhythm")
  )) return true;

  // SEMI só entra se estiver realmente forte; senão vira live/aguardar, não card principal.
  if (mode === "semi" && (p < 72 || proj < 10.8 || ai < 72)) return true;
...
function sortBestGamesForCards(base){
  base.sort((a,b) => {
    ...
    const pa = Number(a?.over95_prob_adj ?? a?.over95_prob ?? 0);
    const pb = Number(b?.over95_prob_adj ?? b?.over95_prob ?? 0);
    if (pb !== pa) return pb - pa;

    const proja = Number(a?.proj_cantos ?? 0);
    const projb = Number(b?.proj_cantos ?? 0);
    if (projb !== proja) return projb - proja;

    const sca = Number(a?.score_adj ?? a?.score ?? 0);
    const scb = Number(b?.score_adj ?? b?.score ?? 0);
...
function hasEnoughGap(candidate, selected, minGapMinutes){
  const candMin = timeToMinutes(candidate?.hora);
  if (candMin === null) return false;

  for (const game of selected) {
    const gameMin = timeToMinutes(game?.hora);
    if (gameMin === null) return false;

    // horários iguais ou muito próximos não entram juntos
    const diff = Math.abs(candMin - gameMin);
    if (diff < minGapMinutes) return false;
  }

  return true;
}

function selectBestWithTimeGap(sortedGames, max, minGapMinutes){
  const selected = [];

  for (const game of sortedGames) {
    if (selected.length >= max) break;
    if (selected.length === 0 || hasEnoughGap(game, selected, minGapMinutes)) {
      selected.push(game);
    }
  }
...
function pickDailyTopGames(top6, allList, limit = DAILY_TOP_GAMES_LIMIT){
  const max = Math.max(1, Number(limit) || 2);
  const seen = new Set();

  let base = [
    ...(Array.isArray(top6) ? top6 : []),
    ...(Array.isArray(allList) ? allList : [])
  ]
    .filter(Boolean)
  ...
  // 🔒 Nunca manda card vermelho/perigoso para os cards principais.
  // Se bloquear algum jogo, o algoritmo puxa o próximo melhor que respeita horário + intervalo.
  if (BLOCK_DANGEROUS_RED_CARDS) {
    base = base.filter(x => !isDangerousRedCardGame(x));
  }

  const sorted = sortBestGamesForCards(base);

  const preferredGap = Math.max(1, Number(MIN_GAME_GAP_MINUTES) || 60);
  const fallbackGap = Math.max(1, Number(FALLBACK_MIN_GAME_GAP_MINUTES) || 30);

  let chosen = selectBestWithTimeGap(sorted, max, preferredGap);
  let appliedGap = preferredGap;

  // Se 60 min for rígido demais para o dia, tenta 30 min.
  if (chosen.length < max && fallbackGap < preferredGap) {
    const relaxed = selectBestWithTimeGap(sorted, max, fallbackGap);
    if (relaxed.length > chosen.length) {
      chosen = relaxed;
      appliedGap = fallbackGap;
    }
  }
  ...
  return chosen.slice(0, max).map((x, i) => ({
    ...
    ai_rank: i + 1,
    daily_rank: i + 1,
    daily_top_rule: 'no_red_cards_premium_leagues_aggressive_corners_with_different_times_and_min_gap',
    min_gap_minutes: appliedGap,
    premium_corner_rank_score: Math.round(premiumCornerRankingScore(x)),
    league_priority_bonus: leaguePriorityBonus(x),
    aggressive_team_bonus: aggressiveCornerTeamsBonus(x),
    ai_reason: x.ai_reason || `sem_card_vermelho_premium_leagues_aggressive_corners_min_${appliedGap}min`
  }));
}
...
function guessLeagueMeta({ id, name, country }){
  ...
  if (nm.includes("premier")) { baseCorners = 11.0; importance = 99; }
  if (nm.includes("championship")) { baseCorners = 10.8; importance = 94; }
  if (nm.includes("bundesliga")) { baseCorners = 11.1; importance = 98; }
  if (nm.includes("eredivisie")) { baseCorners = 10.9; importance = 96; }
  if (nm.includes("ligue 1")) { baseCorners = 10.6; importance = 95; }
  if (nm.includes("serie a") || nm.includes("seria a")) { baseCorners = 10.3; importance = 94; }
  if (nm.includes("la liga")) { baseCorners = 10.5; importance = 93; }
  if (nm.includes("primeira") || nm.includes("liga portugal")) { baseCorners = 10.4; importance = 92; }
  if (nm.includes("belgium") || nm.includes("jupiler")) { baseCorners = 10.5; importance = 91; }

  if (nm.includes("champions")) { baseCorners = 10.4; importance = 90; }
  if (nm.includes("europa league")) { baseCorners = 10.1; importance = 88; }
  if (nm.includes("libertadores")) { baseCorners = 9.8; importance = 86; }
  if (ct.includes("brazil") || ct.includes("brasil") || nm.includes("brasile")) { baseCorners = 10.2; importance = 89; }

  if (nm.includes("ekstraklasa") || nm.includes("czech") || nm.includes("liga i") || nm.includes("hnl")) {
    baseCorners = Math.min(baseCorners, 9.8);
    importance = Math.min(importance, 70);
  }

  if (importance < 78) baseCorners = clamp(baseCorners, 8.9, 10.0);
...
async function buildQuentesList({ date, fresh }) {
  if (!fresh) {
    ...
    if (persisted) return Array.isArray(persisted) ? persisted.map(x => withExtraMarkets(normalizeTeamsOnGame(x))) : persisted;
  }
  ...
  for (const pack of leagueResults) {
    ...
    const cands = (events || []).map((e) => {
      ...
      const lightObj = { league_id: league.id, liga: league.name, casa, fora, proj_cantos: proj, over95_prob: prob, perfil_laterais: perfilLaterais(lateralizacaoIndex(casa, fora, league.baseCorners, proj)) };
      let scoreLight = league.importance + (bigMatch ? 10 : 0) + Math.round((prob - 50) * 0.6);
      scoreLight += leaguePriorityBonus(lightObj);
      scoreLight += aggressiveCornerTeamsBonus(lightObj);
      if (String(lightObj.perfil_laterais) === "LATERAIS_FORTES") scoreLight += 5;
      if (league.id === 99) scoreLight += 3;
...
if (isEuropeanClassic(casaN, foraN)) {
  return null;
    ...
    // ✅ FAVORITO FORA AGORA É REFINADO:
    // 1º/2º fora bloqueia no POWER PRE-GAME; 3º/4º fora pode passar com filtro forte.
    // Os demais favoritos fora continuam bloqueados por segurança.
    const favSideEarly = getFavSideSimple(oddsInfo, posHome, posAway);
    if (favSideEarly === "AWAY" && !allowEliteAwayReplacement) {
      const awayTop34 = Number.isFinite(posAway) && posAway >= 3 && posAway <= 4;
      const awayLeaderVice = Number.isFinite(posAway) && posAway <= 2;
      if (!awayTop34 || awayLeaderVice) {
        if (!awayTop34) return null;
      }
    }
    ...
    if (hasFullBase) {
      ...
      const power95 = powerPreGame95Guard({
        posHome,
        posAway,
        favSide: getFavSideSimple(oddsInfo, posHome, posAway),
        proj_cantos,
        homeRecent,
        awayRecent,
        pressureHits: pressureCheck.hits,
        perfil_laterais,
        isClassic: isEuropeanClassic(casaN, foraN),
        isKnockout: looksLikeKnockout(e)
      });

      for (const f of power95.flags) flags.push(f);
      if (power95.block && !(allowEliteAwayReplacement)) return null;
      ...
      // 🔒 BLOQUEIO: playoff + ritmo baixo
      const playoffLowRhythm = lowRhythmPlayoffBlock({
        eventData: e,
        leagueName: league.name,
        pressureHits: pressureCheck.hits,
        perfil_laterais,
      ...
      if (playoffLowRhythm.block) {
        console.log("BLOQUEADO PLAYOFF/RITMO BAIXO:", casaN, "vs", foraN, playoffLowRhythm.reason);
        return null;
      }
      ...
      score_adj = clamp(
        score_adj_base
        - (trap.penalty || 0)
        - (power95.scorePenalty || 0)
        + (power95.scoreBonus || 0)
      ...
      const obj = {
        ...
        comentario: commentFullFrom({
          ...
          oddsInfo,
          recentCombinedAvg
        }) + (power95.flags.length
          ? ` • Power95: ${power95.flags.slice(0,3).join(", ")}.`
          : "") + (homeResponse?.active
      ...
      const objWithScore = { ...obj, ai_score: aiScoreFromMatch(obj) };
      return withExtraMarkets(objWithScore, { h2h, lastN });
    }

    // ✅ SEM BASE COMPLETA => modo SEMI (mas continua proibindo favorito fora sem aprovação)
    const power95Semi = powerPreGame95Guard({
      posHome,
      posAway,
      favSide: getFavSideSimple(oddsInfo, posHome, posAway),
      proj_cantos,
    ...
    if (playoffLowRhythmSemi.block) {
      console.log("BLOQUEADO SEMI PLAYOFF/RITMO BAIXO:", casaN, "vs", foraN, playoffLowRhythmSemi.reason);
      return null;
    }
    ...
    const semi = {
      ...
      flags,
      power95_class: classifyPower95(score_adj, null, proj_cantos, flags),
    ...
    const semiWithScore = { ...semi, ai_score: aiScoreFromMatch(semi) };
    return withExtraMarkets(semiWithScore, { h2h, lastN });
  }
  ...
  completos = completos
    .map(x => {
      const ai_score = Number.isFinite(x?.ai_score) ? x.ai_score : aiScoreFromMatch(x);
      return { ...x, ai_score, power95_class: x.power95_class || classifyPower95(x.score_adj, ai_score, x.proj_cantos, x.flags || []) };
    })
  ...
  // 🔒 blindagem final: se algum playoff/mata-mata de ritmo baixo escapou por cache/fallback, remove aqui também
  out = out.filter(x => {
    const fakeEvent = {
      match_round: x?.round_raw,
      stage: x?.stage_raw,
      match_stage: x?.stage_raw,
      match_type: x?.type_raw
    };
    const check = lowRhythmPlayoffBlock({
      eventData: fakeEvent,
      leagueName: x?.liga || "",
      pressureHits: x?.real?.pressureHits ?? null,
      perfil_laterais: x?.perfil_laterais,
  ...
  const prefer = litePool.filter(j => {
    if (!LITE_FALLBACK_LEAGUE_IDS.has(j.league_id)) return false;
    const fakeEvent = { match_round: j?.round_raw, stage: j?.stage_raw, match_stage: j?.stage_raw, match_type: j?.type_raw };
    const check = lowRhythmPlayoffBlock({
      eventData: fakeEvent,
      leagueName: j?.liga || "",
      pressureHits: j?.real?.pressureHits ?? null,
      perfil_laterais: j?.perfil_laterais,
      recentCombinedAvg: j?.real?.recentCombinedAvg ?? null,
      proj_cantos: j?.proj_cantos,
      mode: j?.mode || "lite"
    });
    return !check.block;
  });
  ...
  out = out.map(x => {
    const ai_score = Number.isFinite(x?.ai_score) ? x.ai_score : aiScoreFromMatch(x);
    const normalized = normalizeTeamsOnGame({
      ...x,
      ai_score,
      power95_class: x.power95_class || classifyPower95(x.score_adj, ai_score, x.proj_cantos, x.flags || [])
    });
...
/* =========================================================
   ✅ RESULTADO / ESTATÍSTICAS DO JOGO
   Rota usada pelo botão de estatísticas no frontend.
   Não altera a lógica de cantos nem o ranking do pré-jogo.
   ========================================================= */

function resultNum(v){
  const n = Number(String(v ?? "").replace(",", "."));
  return Number.isFinite(n) ? n : null;
}

function pickResultCorners(eventData, side){
  const isHome = side === "home";

  const direct = resultNum(
    isHome
      ? (
          eventData?.match_hometeam_corner ??
          eventData?.match_hometeam_corners ??
          eventData?.hometeam_corner ??
          eventData?.home_corners ??
          eventData?.corners_home
        )
      : (
          eventData?.match_awayteam_corner ??
          eventData?.match_awayteam_corners ??
          eventData?.awayteam_corner ??
          eventData?.away_corners ??
          eventData?.corners_away
        )
  );

  // Alguns provedores retornam 0 mesmo quando os escanteios estão em statistics.
  // Por isso, só aceita direto quando for maior que zero.
  ...
  const possibleBlocks = [
    ...
    eventData?.stats,
    eventData?.match_statistics,
    eventData?.statistics_fulltime,
    eventData?.statistics_1half,
    eventData?.statistics_2half
  ].filter(Boolean);
  ...
  for (const block of possibleBlocks){
    ...
    for (const item of arr){
      ...
      const isCorners =
        ...
        type.includes("escanteio") ||
        type.includes("escanteios");

      if (!isCorners) continue;

      const value = resultNum(
        isHome
          ? (
              item?.home ??
              item?.home_value ??
  ...
  // Último fallback: campos diretos mesmo que sejam 0.
...
function pickResultStat(eventData, side, aliases = []){
  const isHome = side === "home";
  const normAliases = aliases.map(a => String(a || "").toLowerCase());

  const directKeys = isHome
    ? [
        "home_shots", "hometeam_shots", "match_hometeam_shots", "shots_home",
        "home_total_shots", "hometeam_total_shots", "match_hometeam_total_shots",
        "home_shots_total", "hometeam_shots_total", "home_shots_on_goal", "home_shots_on_target",
        "home_dangerous_attacks", "hometeam_dangerous_attacks", "match_hometeam_dangerous_attacks"
      ]
    : [
        "away_shots", "awayteam_shots", "match_awayteam_shots", "shots_away",
        "away_total_shots", "awayteam_total_shots", "match_awayteam_total_shots",
        "away_shots_total", "awayteam_shots_total", "away_shots_on_goal", "away_shots_on_target",
        "away_dangerous_attacks", "awayteam_dangerous_attacks", "match_awayteam_dangerous_attacks"
      ];

  for (const key of directKeys){
    if (!Object.prototype.hasOwnProperty.call(eventData || {}, key)) continue;
    const lk = key.toLowerCase();
    if (!normAliases.some(a => lk.includes(a))) continue;
    const v = resultNum(eventData?.[key]);
  ...
  for (const block of possibleBlocks){
    ...
    for (const item of arr){
      ...
      const matchAlias = normAliases.some(a => type.includes(a));
      if (!matchAlias) continue;
...
/* =========================================================
   MATCH CENTER REAL — STATUS, MINUTO, EVENTOS E CARTÕES
   ========================================================= */
function mcCleanText(v){
  return String(v ?? "").trim();
}

function mcNumReal(v){
  if (v === undefined || v === null || v === "") return null;
  const n = Number(String(v).replace("%", "").replace(",", "."));
  return Number.isFinite(n) ? n : null;
}

function mcFirstFinite(...vals){
  for (const v of vals){
    const n = mcNumReal(v);
...
function mcStatusInfo(eventData, totalGoals){
  const raw = mcCleanText(
    eventData?.match_status ??
    eventData?.status ??
    eventData?.event_status ??
    eventData?.match_live ??
    ""
  );

  const low = raw.toLowerCase();
  const liveFlag = String(eventData?.match_live ?? eventData?.live ?? "").trim();

  const finished =
    low.includes("finished") ||
    low.includes("final") ||
    low.includes("after") ||
    low === "ft" ||
    low === "aet" ||
  ...
  const notStarted =
    !raw || raw === "0" || low.includes("not started") || low.includes("postponed") || low.includes("cancelled");

  const minuteNum = mcFirstFinite(
    eventData?.match_status,
    eventData?.match_minute,
    eventData?.minute,
    eventData?.time
  );
  ...
  if (finished){
    ...
    label = "AO VIVO";
    minute = Number.isFinite(minuteNum) ? minuteNum : null;
    if (minute !== null) period = minute <= 45 ? "1º TEMPO" : "2º TEMPO";
    else if (low.includes("ht")) period = "INTERVALO";
  } else if (low.includes("postponed")){
    label = "ADIADO";
  } else if (low.includes("cancelled") || low.includes("canceled")){
    label = "CANCELADO";
  }

  return { raw, label, finished, live, notStarted: !finished && !live, minute, period };
}

function mcPickPossession(eventData, side){
  const v = pickResultStat(eventData, side, ["possession", "posse", "ball possession", "posse de bola"]);
  return Number.isFinite(v) ? v : null;
}

function mcPickCardsFromStats(eventData, side, color){
  const aliases = color === "red"
    ? ["red cards", "red card", "cartões vermelhos", "cartoes vermelhos", "cartão vermelho", "cartao vermelho"]
    : ["yellow cards", "yellow card", "cartões amarelos", "cartoes amarelos", "cartão amarelo", "cartao amarelo"];
  const v = pickResultStat(eventData, side, aliases);
  return Number.isFinite(v) ? v : 0;
}

function mcEventTeamName(eventData, item){
  const team = mcCleanText(item?.team ?? item?.team_name ?? item?.event_team ?? item?.card_team ?? item?.goal_team ?? item?.substitution_team);
  if (team) return team;
  const home = teamFromEvent(eventData, "home") || "Mandante";
  const away = teamFromEvent(eventData, "away") || "Visitante";
  const side = mcCleanText(item?.home_scorer || item?.home_fault || item?.home_assist || item?.home_player);
  if (side) return home;
  const sideAway = mcCleanText(item?.away_scorer || item?.away_fault || item?.away_assist || item?.away_player);
  if (sideAway) return away;
  return "—";
}

function mcNormalizeEventType(typeRaw, extraRaw=""){
  const t = `${typeRaw} ${extraRaw}`.toLowerCase();
  if (t.includes("goal") || t.includes("gol")) return { type:"goal", label:"⚽ Gol" };
  if (t.includes("corner") || t.includes("escanteio")) return { type:"corner", label:"⚑ Escanteio" };
  if (t.includes("red")) return { type:"red", label:"🟥 Cartão vermelho" };
  if (t.includes("yellow")) return { type:"yellow", label:"🟨 Cartão amarelo" };
  if (t.includes("substitution") || t.includes("substit")) return { type:"sub", label:"🔁 Substituição" };
  return { type:"event", label: mcCleanText(typeRaw || extraRaw) || "Evento" };
}

function mcNormalizeEvents(eventData){
  const blocks = [
    eventData?.goalscorer,
    eventData?.cards,
    eventData?.substitutions,
    eventData?.lineup,
    eventData?.events,
    eventData?.match_events
  ].filter(Boolean);
  ...
  for (const block of blocks){
    if (Array.isArray(block)){
      for (const item of block){
        ...
        const minute = mcCleanText(item?.time ?? item?.minute ?? item?.match_minute ?? item?.event_time ?? item?.card_time ?? item?.score_time);
        const rawType = mcCleanText(item?.type ?? item?.event_type ?? item?.card ?? item?.score ?? item?.info ?? item?.event_name);
        const rawDetail = mcCleanText(item?.detail ?? item?.card_type ?? item?.event_detail ?? item?.comment);
        const norm = mcNormalizeEventType(rawType, rawDetail);
        const team = mcEventTeamName(eventData, item);
        if (!minute && norm.type === "event" && team === "—") continue;
        out.push({ minute: minute ? `${minute}'`.replace("''", "'") : "—", type:norm.type, label:norm.label, team });
      }
      ...
      for (const arr of Object.values(block)){
        if (Array.isArray(arr)){
          for (const item of arr){
            if (!item || typeof item !== "object") continue;
            const minute = mcCleanText(item?.time ?? item?.minute ?? item?.match_minute ?? item?.event_time);
            const rawType = mcCleanText(item?.type ?? item?.event_type ?? item?.card_type ?? item?.event_name);
            const norm = mcNormalizeEventType(rawType);
            const team = mcEventTeamName(eventData, item);
            out.push({ minute: minute ? `${minute}'`.replace("''", "'") : "—", type:norm.type, label:norm.label, team });
          }
        }
  ...
  // tira duplicados simples e ordena do mais recente para o mais antigo
  const seen = new Set();
  return out.filter(e => {
    const k = `${e.minute}|${e.label}|${e.team}`;
    if (seen.has(k)) return false;
    seen.add(k);
    return true;
  }).sort((a,b) => (parseInt(b.minute) || 0) - (parseInt(a.minute) || 0)).slice(0, 12);
}

function mcBuildCards(eventData){
  const events = mcNormalizeEvents(eventData);
  const yhStats = mcPickCardsFromStats(eventData, "home", "yellow");
  const yaStats = mcPickCardsFromStats(eventData, "away", "yellow");
  const rhStats = mcPickCardsFromStats(eventData, "home", "red");
  const raStats = mcPickCardsFromStats(eventData, "away", "red");

  const home = teamFromEvent(eventData, "home") || "";
  const away = teamFromEvent(eventData, "away") || "";
  const hKey = normTeamKey(home);
  const aKey = normTeamKey(away);

  let yellow_home = yhStats, yellow_away = yaStats, red_home = rhStats, red_away = raStats;
  for (const e of events){
    const tk = normTeamKey(e.team);
    const isHome = tk && hKey && (tk.includes(hKey) || hKey.includes(tk));
    const isAway = tk && aKey && (tk.includes(aKey) || aKey.includes(tk));
    if (e.type === "yellow"){
      if (isHome) yellow_home++;
      if (isAway) yellow_away++;
    }
    if (e.type === "red"){
      if (isHome) red_home++;
      if (isAway) red_away++;
...
function buildMatchResultPayload(eventData){
  ...
  const homeGoals = resultNum(
    eventData?.match_hometeam_score ??
    eventData?.match_hometeam_ft_score ??
    eventData?.home_score ??
    eventData?.score_home ??
    eventData?.goals_home
  );

  const awayGoals = resultNum(
    eventData?.match_awayteam_score ??
    eventData?.match_awayteam_ft_score ??
    eventData?.away_score ??
    eventData?.score_away ??
  ...
  return {
    ...
    not_started: statusInfo.notStarted,
    ...
    shots: {
      ...
      away: awayShots,
      total: totalShots,
      on_target_home: homeShotsOn,
      on_target_away: awayShotsOn,
      on_target_total: totalShotsOn
    },
...
app.get("/match_result", async (req, res) => {
  const matchId = String(req.query.match_id || req.query.id || "").trim();

  if (!matchId){
    return res.status(400).json({
      error: "match_id obrigatório"
    });
  }
  ...
  try {
    ...
    if (!result){
      return res.status(404).json({
        error: "Resultado não encontrado para este jogo",
        match_id: matchId
      });
    }
...
/* =========================================================
   MATCH CENTER — ALIAS PARA DASHBOARD LIVE/PÓS-JOGO
   Usa a mesma base de /match_result, sem alterar o motor principal.
   ========================================================= */
app.get("/match_center", async (req, res) => {
  ...
  try {
    ...
    if (!result){
      return res.status(404).json({
        error: "Match Center não encontrou dados para este jogo",
        match_id: matchId
      });
    }
...
app.get("/quentes", async (req, res) => {
  ...
  try {
    ...
    const dailyTop = pickDailyTopGames(top6, out, dailyTopLimitForDate(date));

    const humanMap = await aiHumanAnalyzeTop6(dailyTop, date);

    const dailyTop_enriched = dailyTop.map(x => ({
      ...x,
      ai_human: humanMap.get(stableKey(x)) || humanFallbackFromGame(x)
    }));

    if (onlyTop) return res.json(dailyTop_enriched);

    const topKeys = new Set(dailyTop_enriched.map(stableKey));
    const rest = out.filter(x => !topKeys.has(stableKey(x)));

    // ✅ retorna os melhores com horários espaçados + resto
    return res.json([...dailyTop_enriched, ...rest]);
...
// =========================================================
// ADMIN LIVE GAMES — JOGOS DE HOJE EM TEMPO REAL
// =========================================================
app.get("/admin/live-games", async (req, res) => {

  const date = req.query.date || toISODate();
  const fresh = String(req.query.fresh || "1") === "1";

  try{

    const games = await buildQuentesList({ date, fresh });
    ...
    for(const game of (Array.isArray(games) ? games : [])){
      ...
      const home = game.casa || game.home || "Time A";
      const away = game.fora || game.away || "Time B";
      const time = game.hora || game.time || "—";
      const league = game.liga || game.league_name || game.league?.name || "Liga";

      const key = `${home}-${away}-${league}-${time}`
        .toLowerCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .replace(/\s+/g, " ")
    ...
    const liveGames = uniqueGames.slice(0, 30).map(game => {

      return {
        id: game.match_id || game.id || game.event_key || null,
        home: game.casa || game.home || "Time A",
        away: game.fora || game.away || "Time B",
        league: game.liga || game.league_name || game.league?.name || "Liga",
        time: game.hora || game.time || "—",
        status: game.status || game.match_status || "Pré-jogo",
        projectedCorners: game.proj_cantos ?? game.projCorners ?? null,
        probability: game.over95_prob_adj ?? game.over95_prob ?? null,
        aiScore: game.ai_score ?? game.score_adj ?? game.score ?? null,
    ...
    console.error("ADMIN LIVE GAMES ERROR:", err);

    res.status(500).json({
      ok: false,
      error: "Erro ao carregar jogos de hoje",
      details: String(err?.message || err)
    });
...
// =========================================================
// ADMIN LIVE NOW — JOGOS AO VIVO
// =========================================================
app.get("/admin/live-now", async (req, res) => {
  ...
  try{
    ...
    const liveGames = (Array.isArray(data) ? data : [])
      ...
      .map(game => ({
        ...
        id:
          game.match_id || game.event_key || null,

        home:
          game.match_hometeam_name || "Time A",

        away:
          game.match_awayteam_name || "Time B",

        league:
          game.league_name || "Liga",

        score:
          `${game.match_hometeam_score ?? 0} x ${game.match_awayteam_score ?? 0}`,

        status:
          game.match_status || "AO VIVO",
    ...
    res.status(500).json({
      ok:false,
      error:"Erro ao carregar jogos ao vivo",
      details:String(err?.message || err)
    });
...
app.post("/auth/login", (req, res) => {
  ...
  return res.json({
    ok:true,
    user:{ email, plan:"premium" },
    token:"corners-premium-demo"
  });
});
...
// Rota para a tela de filtros/mercados.
// Mantém o motor principal intacto, mas entrega uma lista ampla para gols,
// escanteios e cartões no front-end.
app.get("/mercados", async (req, res) => {
  ...
  try {
    const games = await buildQuentesList({ date, fresh });
    const out = (Array.isArray(games) ? games : [])
      .map(game => withExtraMarkets(normalizeTeamsOnGame(game)))
      .slice(0, 60);
    ...
    const fallback = !fresh ? readPersist(date) : null;
    if (fallback) {
      return res.json((Array.isArray(fallback) ? fallback : []).map(game => withExtraMarkets(game)));
    }
    res.status(500).json({
      error:"Erro ao carregar mercados",
      details:String(err?.message || err)
    });
  }
});
...
app.listen(PORT, () => {
  ...
  console.log(`- IA Top do dia: 2 úteis / 3 fim de semana, ligas premium + times agressivos, horários diferentes e intervalo mínimo: /quentes_ai?date=YYYY-MM-DD&fresh=1`);
