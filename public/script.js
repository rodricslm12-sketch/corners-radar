...
async function openMatch(game) {
 ...
 content.innerHTML = `
   <div class="cpMobileMatchLoading">
     <span></span>
     <b>Carregando dados reais da partida...</b>
   </div>`;

 const rawStatus = clean(
   game.raw?.status ??
   game.raw?.match_status ??
   game.raw?.status_raw ??
   game.raw?.event_status ??
   game.status,
   ""
 ).toLowerCase();

 const finishedByStatus = /finish|finished|encerr|ft|final|after match|ended/.test(rawStatus);
 const liveByStatus = /live|andamento|1h|2h|interval|halftime|in play|playing/.test(rawStatus);

 try {
   const sourceGame = game.raw || game;
   const allGames = activeList().map(item => item.raw || item);

   if (typeof window.updateDesktopMatchRail === "function") {
     await window.updateDesktopMatchRail(sourceGame, allGames);
   } else if (typeof window.openMatchCenter === "function") {
     await window.openMatchCenter(sourceGame);
   }

   await new Promise(resolve => setTimeout(resolve, 420));

   const rail = $("#desktopMatchRail") || $(".dashboardRightRail");
   const railText = String(rail?.textContent || "").toLowerCase();

   const finished = finishedByStatus ||
     /encerrado|finalizado|fim de jogo|resultado final|ft/.test(railText);

   const live = !finished && (
     liveByStatus ||
     /ao vivo|em andamento|intervalo|1º tempo|2º tempo|match center ao vivo/.test(railText)
   );

   if (finished) {
     if (title) title.textContent = "ESTATÍSTICAS FINAIS";
     if (subtitle) subtitle.textContent = "Resultado, cartões, escanteios, chutes e números completos";
   } else if (live) {
     if (title) title.textContent = "MATCH CENTER AO VIVO";
     if (subtitle) subtitle.textContent = "Placar, pressão, eventos e estatísticas em tempo real";
   } else {
     if (title) title.textContent = "ANÁLISE PRÉ-JOGO";
     if (subtitle) subtitle.textContent = "Projeções, histórico e dados antes da partida";
   }

   if (rail && rail.children.length) {
     const clone = rail.cloneNode(true);
     clone.removeAttribute("id");
     clone.classList.add("cpMobileRailClone");

     clone.querySelectorAll("[id]").forEach(element => element.removeAttribute("id"));
     clone.querySelectorAll("button[disabled]").forEach(button => button.remove());
   ...
   if (!live && !finished && typeof window.renderMobilePregame === "function") {
     window.renderMobilePregame(sourceGame);
     return;
   }

   throw new Error("O Match Center não retornou conteúdo.");
 } catch (error) {
   console.warn("[Corner Pro Match Center]", error);

   const status = rawStatus || clean(game.status, "Pré-jogo");
   const finished = finishedByStatus;
   const live = liveByStatus;
   ...
   const homeCorners = stat(raw.home_corners, raw.corners_home, raw.stats?.home?.corners);
   const awayCorners = stat(raw.away_corners, raw.corners_away, raw.stats?.away?.corners);
   const homeCards = stat(raw.home_cards, raw.cards_home, raw.stats?.home?.cards);
   const awayCards = stat(raw.away_cards, raw.cards_away, raw.stats?.away?.cards);
   const homeShots = stat(raw.home_shots, raw.shots_home, raw.stats?.home?.shots);
   const awayShots = stat(raw.away_shots, raw.shots_away, raw.stats?.away?.shots);
   ...
   content.innerHTML = `
     <section class="cpV8MatchHero">
       ...
       <b>${escapeHtml(String(status).toUpperCase())}</b>
     </section>
     ...
     <section class="cpV8MatchRead">
       ...
       <p>${
         finished
           ? "A partida terminou. As estatísticas finais disponíveis são exibidas acima."
           : live
             ? "A partida está em andamento. Os números serão atualizados conforme o servidor enviar novos eventos."
             : "A partida ainda não começou. Consulte projeção, mercado, confiança e contexto pré-jogo."
       }</p>
     </section>`;
 }
}
