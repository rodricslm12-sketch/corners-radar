...
/* =========================================================
   HISTÓRICO PÓS-JOGO — ABA SIMPLES
   - Usa /historico do server.js.
   - Não mexe no motor principal de pré-jogo.
   ========================================================= */
(function installHistoryPanel(){
  function hSafe(v, fallback="—"){
    const s = String(v ?? "").trim();
    return s && s !== "undefined" && s !== "null" ? s : fallback;
  }

  function hEscape(v){
    return hSafe(v, "").replace(/[&<>'"]/g, ch => ({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#039;",'"':"&quot;"}[ch]));
  }
  ...
  function ensureHistoryPanel(){
    ...
    panel.innerHTML = `
      <div class="panel-title">
        <h2>📊 HISTÓRICO DE JOGOS FINALIZADOS</h2>
        <span id="countHistory">0</span>
      </div>
      <div class="historyTopBar">
        <div><strong>Resultados salvos automaticamente</strong><span>Quando o jogo termina, o Match Center envia o resultado para cá.</span></div>
        <button id="refreshHistoryBtn" class="btnOutline" type="button">Atualizar histórico</button>
      </div>
      <div id="historyGames" class="history-games"><div class="empty">Nenhum jogo finalizado salvo ainda.</div></div>
    `;

    const quote = document.querySelector(".quoteSteve");
    if (quote && quote.parentNode) quote.parentNode.insertBefore(panel, quote);
    else content.appendChild(panel);
    return panel;
  }

  function setHeaderForHistory(){
    ...
    if (sub) sub.textContent = "JOGOS FINALIZADOS E RESULTADO DO MERCADO";
    if (panelTitle) panelTitle.textContent = "● ESCANTEIOS POR TIME";
  }

  function showOnlyHistory(){
    const pre = document.getElementById("prePanel");
    const filters = document.getElementById("filtersPanel");
    const history = ensureHistoryPanel();
    if (pre) pre.style.display = "none";
    if (filters) filters.style.display = "none";
    history.style.display = "block";
    document.querySelectorAll("[data-tab]").forEach(x => x.classList.remove("active", "is-active"));
    document.querySelectorAll('[data-tab="history"]').forEach(x => x.classList.add("active", "is-active"));
    setHeaderForHistory();
  }

  function restorePanelsForNonHistory(){
  ...
  function renderHistory(items){
    const box = document.getElementById("historyGames");
    const count = document.getElementById("countHistory");
    if (count) count.textContent = String(items?.length || 0);
    if (!box) return;

    if (!Array.isArray(items) || !items.length){
      box.innerHTML = `<div class="empty">Nenhum jogo finalizado salvo ainda. Abra o Match Center de um jogo encerrado para ele entrar aqui.</div>`;
      return;
    }

    box.innerHTML = items.map(item => {
      const hit = String(item?.resultado || "").includes("bateu") && !String(item?.resultado || "").includes("nao");
      const loss = String(item?.resultado || "").includes("nao");
      const statusText = hit ? "✅ BATEU" : loss ? "❌ NÃO BATEU" : "⏳ AGUARDANDO";
      return `<article class="historyCard ${loss ? "is-loss" : ""}">
        <div class="historyCardTop">
          <div class="historyLeague">${hEscape(item?.liga || "Liga")} · ${hEscape(item?.data || "")}</div>
          <div class="historyStatus ${loss ? "loss" : ""}">${statusText}</div>
        </div>
        <div class="historyTeams">
          <span>${hEscape(item?.casa)}</span><i class="historyVs">VS</i><span>${hEscape(item?.fora)}</span>
        </div>
        <div class="historyScore">
          <div class="historyMini"><span>Placar</span><b>${hEscape(item?.placar)}</b></div>
          <div class="historyMini"><span>Cantos</span><b>${hEscape(item?.corners)}</b></div>
          <div class="historyMini"><span>Total</span><b>${hEscape(item?.total_corners)}</b></div>
        </div>
        <div class="historyFooter">
          <span>Mercado: <b>${hEscape(item?.mercado || "+9.5 CANTOS")}</b></span>
          <span>${item?.odd ? "Odd " + hEscape(item.odd) : hEscape(item?.hora || "")}</span>
        </div>
      </article>`;
    }).join("");
  }
  ...
  async function loadHistory(){
    ensureHistoryPanel();
    const box = document.getElementById("historyGames");
    if (box) box.innerHTML = `<div class="empty">Carregando histórico...</div>`;
    try{
      const res = await fetch("/historico?limit=120");
      const data = await res.json();
      renderHistory(Array.isArray(data) ? data : (data.items || []));
    }catch(err){
      if (box) box.innerHTML = `<div class="empty">Não consegui carregar o histórico agora. Verifique se o server.js atualizado está rodando.</div>`;
    }
  }

  document.addEventListener("click", function(ev){
    const historyBtn = ev.target.closest('[data-tab="history"]');
    if (historyBtn){
      ev.preventDefault();
      ev.stopPropagation();
      showOnlyHistory();
    ...
    if (ev.target.closest("#refreshHistoryBtn")){
      ev.preventDefault();
      loadHistory();
      return;
    }

    const otherTab = ev.target.closest("[data-tab]");
    if (otherTab && otherTab.getAttribute("data-tab") !== "history"){
      restorePanelsForNonHistory();
    }
  }, true);

  document.addEventListener("DOMContentLoaded", ensureHistoryPanel);
})();

/* =========================================================
   PATCH FINAL — RENDERIZAÇÃO IGUAL AO MODELO COM MATCH CENTER
   ========================================================= */
(function(){
  const CR_MARKETS = [
    {key:"all", label:"TODOS", icon:""},
    {key:"btts", label:"AMBAS MARCAM", icon:"👥"},
    {key:"over15", label:"+1.5 GOLS", icon:"⚽"},
    {key:"over25", label:"+2.5 GOLS", icon:"⚽"},
    {key:"over35", label:"+3.5 GOLS", icon:"⚽"},
    {key:"over45", label:"+4.5 GOLS", icon:"⚽"},
    {key:"corners95", label:"+9.5 ESCANTEIOS", icon:"⚑"},
    {key:"corners105", label:"+10.5 ESCANTEIOS", icon:"⚑"},
    {key:"corners115", label:"+11.5 ESCANTEIOS", icon:"⚑"},
    {key:"cards25", label:"CARTÕES", icon:"🟨"},
    {key:"cardsTeam", label:"CARTÕES POR TIME", icon:"🟨"},
    {key:"noCard28", label:"NÃO LEVAM CARTÃO ATÉ 28'", icon:"🛡️"},
    {key:"last5", label:"ÚLTIMOS 5 JOGOS", icon:"📊"}
  ];

  function S(v,f="—"){ return (v===undefined||v===null||v==="")?f:String(v); }
  function N(v,d=1){ const n=Number(v); return Number.isFinite(n)?n.toFixed(d).replace(".0",""):"—"; }
  function esc(v){ return String(v??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;"); }
  function getHome(j){ return S(j?.casa || j?.home || j?.home_team,"Time A"); }
  function getAway(j){ return S(j?.fora || j?.away || j?.away_team,"Time B"); }
  function getLeague2(j){ return S(j?.liga || j?.league_name || j?.league?.name,"Liga"); }
  function getD(){ return lastMarketDateYMD || lastDateYMD || dateInput?.value || ""; }
  function getTime2(j){ try{return timeOnlyAM(getD(), S(j?.hora || j?.time,"—"));}catch(e){return S(j?.hora || j?.time,"—");} }
  function getDateBR(){ const d=getD(); if(/^\d{4}-\d{2}-\d{2}$/.test(d)){const [y,m,dd]=d.split("-"); return `${dd}/${m}`;} return ""; }
  function prob2(j,key){
    const m=j?.markets||{}; const p=m?.prob||{};
    const proj=Number((typeof getProj==="function"?getProj(j):j?.proj_cantos) || 10);
    const base=Number((typeof getProb==="function"?getProb(j):j?.over95_prob_adj) || 60);
    const seed=Math.abs(`${getHome(j)}${getAway(j)}${getTime2(j)}`.split("").reduce((a,c)=>a+c.charCodeAt(0),0));
    const cards=Math.max(42,Math.min(84,Math.round(52+(proj-9.6)*5+(base-62)*.18+(seed%9))));
    const map={
      all:Math.max(base,Number(p.over25||0),cards), btts:Number(p.btts||56), over15:Number(p.over15||68), over25:Number(p.over25||58), over35:Number(p.over35||42), over45:Math.max(20,Number(p.over35||42)-14),
      corners95:base, corners105:Math.max(3,base-8), corners115:Math.max(2,base-18), cards25:cards, cardsTeam:Math.max(35,cards-4), noCard28:Math.max(38,Math.min(76,109-cards)), last5:base
    };
    return Math.round(Number(map[key] ?? map.all) || 0);
  }
  function marketOk(j,key){ if(!key||key==="all") return true; const p=prob2(j,key); return p>= (key==="corners115"?50:key==="over35"?48:key==="noCard28"?55:52); }
  function logoFor(name){ const n=String(name||"").toLowerCase(); if(n.includes("real madrid"))return"👑"; if(n.includes("liverpool"))return"🔴"; if(n.includes("city")||n.includes("manchester"))return"🔵"; if(n.includes("arsenal"))return"🔴"; if(n.includes("juventus"))return"⚪"; if(n.includes("bayern"))return"🔴"; if(n.includes("psg"))return"🔵"; if(n.includes("inter"))return"🔵"; return "⚽"; }
  function matchId(j){ return S(j?.match_id || j?.id || j?.event_key || ""); }
  function odd(j){ const o=Number(j?.odds?.fav?.odd ?? j?.odd ?? j?.best_odd ?? NaN); return Number.isFinite(o)?o.toFixed(2):"1.68"; }
  function renderTopGame(j,idx,key){
    const home=getHome(j), away=getAway(j), p=prob2(j,key), proj=Number((typeof getProj==="function"?getProj(j):j?.proj_cantos)||10.8);
    const mediaCasa=Number(j?.real?.homeRecent?.cornersForAvg ?? j?.home_corners_avg ?? 5.9);
    const mediaFora=Number(j?.real?.awayRecent?.cornersForAvg ?? j?.away_corners_avg ?? 4.9);
    const h2h=Number(j?.h2h_avg ?? j?.real?.h2hAvg ?? (proj+.3));
    const conf=Math.max(69,Math.min(88,Math.round(p+12)));
    return `<article class="cr-top-card">
      <div class="cr-card-ribbon">${idx===0?"MELHOR DO DIA":"ALTERNATIVA TOP"}</div>
      <div class="cr-time"><b>${getTime2(j)}</b><span>${getDateBR()}</span><small>🏳️ ${idx===0?"England":"Spain"}<br>${getLeague2(j)}</small></div>
      <div class="cr-team home"><div class="cr-logo">${logoFor(home)}</div><strong title="${esc(home)}">${esc(home)}</strong><em>Casa</em></div>
      <div class="cr-proj"><span class="cr-pill">Full forte</span><div class="cr-proj-box"><span>Proj. escanteios</span><b>${N(proj,1)}</b><small>Linha: +9.5</small></div></div>
      <div class="cr-team away"><div class="cr-logo">${logoFor(away)}</div><strong title="${esc(away)}">${esc(away)}</strong><em>Fora</em></div>
      <div class="cr-prob"><span>Probabilidade</span><b>${p}%</b></div>
      <div class="cr-odd"><span>Odd média</span><b>${odd(j)}</b></div>
      <div class="cr-metrics">
        <div class="cr-metric"><span>Média casa</span><b>${N(mediaCasa,1)}</b></div>
        <div class="cr-metric"><span>Média fora</span><b>${N(mediaFora,1)}</b></div>
        <div class="cr-metric"><span>H2H escanteios</span><b>${N(h2h,1)}</b></div>
        <div class="cr-metric green"><span>Ritmo IA</span><b>↗ Alto</b></div>
        <div class="cr-metric ${idx===1?"yellow":"green"}"><span>Alerta IA</span><b>${idx===1?"🛡 Médio":"🛡 Baixo"}</b></div>
        <div class="cr-metric green"><span>Confiança IA</span><b>${conf}%</b></div>
        <div class="cr-metric"><span>Nota IA</span><b>⭐ ${idx===0?"9.1":"8.6"}</b></div>
      </div>
      <div class="cr-details"><button type="button" data-cr-detail="${idx}">Ver detalhes</button></div>
    </article>`;
  }
  function renderTableRow(j,idx,key){
    const p=prob2(j,key), proj=Number((typeof getProj==="function"?getProj(j):j?.proj_cantos)||10);
    return `<tr><td>${getTime2(j)}</td><td><b>${esc(getHome(j))}</b> <span class="cr-vs-mini">vs</span> ${esc(getAway(j))}</td><td>${esc(getLeague2(j))}</td><td><b>${N(proj,1)}</b></td><td>+9.5</td><td class="cr-green"><b>${p}%</b></td><td>${odd(j)}</td><td class="${idx%2?"cr-yellow":"cr-green"}"><b>↗ ${idx%2?"Médio":"Alto"}</b></td><td class="cr-green"><b>🛡 Baixo</b></td><td class="cr-green"><b>${Math.max(69,p+8)}%</b></td><td class="cr-star">⭐ ${(8.2-idx*.2).toFixed(1)}</td></tr>`;
  }
  function renderMatchCenter(j){
    const home=getHome(j), away=getAway(j);
    return `<section class="cr-side-panel cr-match-card"><div class="cr-side-head"><h3>Match Center</h3><span class="cr-live">AO VIVO</span></div><div class="cr-score-row"><div class="cr-score-team"><div class="cr-logo">${logoFor(home)}</div><span>${esc(home).split(" ").slice(0,2).join(" ")}</span></div><div><div class="cr-score">2 - 1</div><span class="cr-minute">68:24<br>2º TEMPO</span></div><div class="cr-score-team"><div class="cr-logo">${logoFor(away)}</div><span>${esc(away).split(" ").slice(0,2).join(" ")}</span></div></div><div class="cr-corners-title">Escanteios</div><div class="cr-corner-bar"><span>6</span><div class="cr-bars-live"></div><span>4</span></div><div class="cr-timeline"></div><button class="cr-match-open" type="button" data-open-match-center="0" data-match-id="${esc(matchId(j))}" data-home="${esc(home)}" data-away="${esc(away)}" data-league="${esc(getLeague2(j))}" data-time="${esc(getTime2(j))}">Abrir Match Center</button></section>`;
  }
  function renderCrFilters(key){
    return `<section class="cr-side-panel"><div class="cr-side-head"><h3>Filtros de Mercado</h3><span class="cr-badge-num">12</span></div><div class="cr-filter-grid">${CR_MARKETS.map(m=>`<button type="button" class="cr-filter-btn ${key===m.key?"active":""}" data-cr-market="${m.key}">${m.icon} ${m.label}</button>`).join("")}</div><div class="cr-sort"><label>Ordenar por</label><select id="crSortSelect"><option value="market" ${filterSortMode==="market"?"selected":""}>Maior força do filtro</option><option value="time" ${filterSortMode==="time"?"selected":""}>Horário dos jogos</option><option value="corners" ${filterSortMode==="corners"?"selected":""}>Mais escanteios</option></select></div></section>`;
  }

  window.renderMarketFilters = renderMarketFilters = function(){
    if(!top1El) return;
    const key = (typeof getActiveMarket==="function" ? getActiveMarket() : activeMarketFilter) || "corners95";
    let list = Array.isArray(lastMarketGames)&&lastMarketGames.length ? lastMarketGames : lastRawGames;
    list = (typeof dedupeList==="function"?dedupeList(list):list).slice();
    let filtered = list.filter(j=>marketOk(j,key));
    filtered.sort((a,b)=>{
      if(filterSortMode==="time"){
        try{const ma=getMatchMinutesAM(a,getD()), mb=getMatchMinutesAM(b,getD()); if(ma!==null&&mb!==null&&ma!==mb)return ma-mb;}catch(e){}
      }
      if(filterSortMode==="corners") return (Number(typeof getProj==="function"?getProj(b):b?.proj_cantos)||0)-(Number(typeof getProj==="function"?getProj(a):a?.proj_cantos)||0);
      return prob2(b,key)-prob2(a,key);
    });
    const top=filtered.slice(0,2);
    const others=filtered.slice(2,6);
    const first=top[0] || list[0] || {};
    top1El.closest(".panel")?.classList.remove("is-market-scroll-panel");
    top1El.innerHTML = `<div class="cr-pro-shell"><main class="cr-pro-main"><div class="cr-pro-title"><h1>2 JOGOS DO DIA</h1><span>Segunda a sexta: 1 entrada cedo + 1 entrada mais tarde</span></div><section class="cr-top-cards">${top.length?top.map((j,i)=>renderTopGame(j,i,key)).join(""):`<div class="empty">Nenhum jogo encontrado para esse mercado nesta data.</div>`}</section><section class="cr-table"><div class="cr-table-title"><h2>Outros jogos em destaque</h2><span class="cr-count">${others.length}</span></div><table><thead><tr><th>Horário</th><th>Partida</th><th>Liga</th><th>Proj. escanteios</th><th>Linha</th><th>Prob.</th><th>Odd</th><th>Ritmo</th><th>Alerta</th><th>Conf.</th><th>Nota</th></tr></thead><tbody>${others.map((j,i)=>renderTableRow(j,i,key)).join("")}</tbody></table><button class="cr-open-all" type="button">Ver todos os jogos⌄</button></section><section class="cr-bottom"><div class="cr-ai-box"><div class="cr-ai-icon">↯</div><div><strong>Análise inteligente <b>em tempo real</b></strong><span>${esc(getHome(first))} mantém alta pressão e média forte de cantos em casa.<br>Probabilidade de mais de 9.5 escanteios no final: <b>${prob2(first,key)}%</b></span></div></div><div class="cr-small-card"><strong>Ritmo de escanteios</strong><b>↗ Alto</b></div><div class="cr-small-card yellow"><strong>Alerta IA <b>🛡 Baixo</b></strong><p>Jogo com tendência constante de cantos.</p></div></section></main><aside class="cr-right">${renderCrFilters(key)}${renderMatchCenter(first)}</aside></div>`;
    window.__premiumFilteredGames = filtered;
    if(countTop) countTop.textContent=String(filtered.length);
    bindCrEvents();
  };
  function bindCrEvents(){
    top1El?.querySelectorAll("[data-cr-market]").forEach(btn=>btn.addEventListener("click",()=>{ if(typeof setActiveMarket==="function") setActiveMarket(btn.dataset.crMarket||"all"); else activeMarketFilter=btn.dataset.crMarket||"all"; renderMarketFilters(); }));
    const sort=top1El?.querySelector("#crSortSelect"); if(sort) sort.addEventListener("change",()=>{filterSortMode=sort.value||"market"; renderMarketFilters();});
    top1El?.querySelectorAll("[data-open-match-center]").forEach(btn=>btn.addEventListener("click",ev=>{ev.stopPropagation(); if(typeof openMatchCenter==="function") openMatchCenter({matchId:btn.dataset.matchId,home:btn.dataset.home,away:btn.dataset.away,league:btn.dataset.league,time:btn.dataset.time});}));
  }
  document.addEventListener("DOMContentLoaded",()=>{
    document.querySelectorAll('[data-tab="filters"]').forEach(x=>x.classList.remove("active"));
    document.querySelectorAll('[data-tab="corners"]').forEach(x=>x.classList.add("active"));
  });
})();