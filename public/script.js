/* =========================================================
   CORNER PRO MOBILE CONTROLLER V8
   Ativa: data, cards Cantos/Gols/Cartões, mercados e Match Center.
   ========================================================= */
   (() => {
    "use strict";
  
    if (window.__cpMobileControllerV8) return;
    window.__cpMobileControllerV8 = true;
  
    // Impede o loader simplificado V6 de substituir este controlador.
    ...
    function clean(v, fallback = "") {
      ...
      return t && !["undefined", "null", "NaN"].includes(t) ? t : fallback;
    }
    ...
    function team(g, side) {
      if (side === "home") return clean(g.casa ?? g.home ?? g.home_name ?? g.mandante, "Casa");
      return clean(g.fora ?? g.away ?? g.away_name ?? g.visitante, "Fora");
    }
  
    function time(g) {
      const t = clean(g.hora_manaus ?? g.hora ?? g.match_time ?? g.time, "--:--");
    ...
    function confidence(g, type) {
      let v;
      if (type === "goals") {
        v = num(g.goals?.confidence, g.goals_confidence, g.over25_prob, g.goal_prob);
      } else if (type === "cards") {
        v = num(g.cards?.confidence, g.cards_confidence, g.over35_cards_prob, g.card_prob);
      } else {
        v = num(g.over95_prob_adj, g.over95_prob, g.confidence, g.ai_score, g.score);
      }
      if (v === null) v = 56;
      if (v > 0 && v <= 1) v *= 100;
      while (v > 100) v /= 10;
    ...
    function line(g, type) {
      if (type === "goals") {
        return clean(g.goals?.line ?? g.goals_line ?? g.goal_line, "OVER 2.5").toUpperCase();
      }
      if (type === "cards") {
        return clean(g.cards?.line ?? g.cards_line ?? g.card_line, "OVER 3.5").toUpperCase();
      }
      const p = num(g.proj_cantos, g.projCorners, g.projection) ?? 10.2;
      return p >= 12 ? "OVER 11.5" : p >= 11 ? "OVER 10.5" : "OVER 9.5";
    }
  
    function normalize(g, type, i) {
      return {
        raw: g,
        id: clean(g.match_id ?? g.fixture_id ?? g.id, `${team(g,"home")}|${team(g,"away")}|${i}`),
        home: team(g, "home"),
        away: team(g, "away"),
        time: time(g),
    ...
    function buildMarket(raw, type) {
      const seen = new Set();
      return raw.map((g, i) => normalize(g, type, i))
    ...
    async function loadData() {
      ...
      const [q, m] = await Promise.allSettled([
        getJson(`/quentes?date=${encodeURIComponent(date)}&mobile=1&_mobile=${stamp}`),
        getJson(`/mercados?date=${encodeURIComponent(date)}&_mobile=${stamp}`)
      ]);
  
      const qGames = q.status === "fulfilled" ? extract(q.value) : [];
      const mGames = m.status === "fulfilled" ? extract(m.value) : [];
      const raw = mGames.length ? mGames : qGames;
  
      state.all = raw;
      state.corners = buildMarket(qGames.length ? qGames : raw, "corners");
    ...
    function activeList() {
      return state[state.activeMarket] || [];
    }
  
    function setLoading(on) {
      const card = $("#cpHomeBest");
      if (card) {
        card.classList.toggle("is-loading-initial", on);
        card.setAttribute("aria-busy", on ? "true" : "false");
      }
      const load = $("#cpHomeInitialLoading");
    ...
    function renderActive() {
      ...
      $$(".cpHomeMarketDots [data-cp-home-dot]").forEach(btn => {
        btn.classList.toggle("active", btn.dataset.cpHomeDot === state.activeMarket);
      });
  
      const meta = MARKET[state.activeMarket];
      const best = list[0];
  
      if (!best) {
        $("#cpHomeGames").innerHTML = `<div class="cpHomeEmptyState"><strong>SEM JOGOS</strong><small>Nenhuma partida encontrada para esta data.</small></div>`;
        return;
      }
  
      $("#cpHomeBestTitle").textContent = meta.title;
      $("#cpHomeBestMarket").textContent = best.line;
      $("#cpHomeBestTime").textContent = best.time;
      $("#cpHomeBestHome").textContent = best.home;
      $("#cpHomeBestAway").textContent = best.away;
      $("#cpHomeBestConfidence").textContent = `${best.confidence}%`;
      $("#cpHomeMatchTeams").textContent = `${best.home} × ${best.away}`;
  
      const open = $("#cpHomeBestOpen");
      open.disabled = false;
      open.querySelector(".cpHomeBestOpenText").textContent = "VER ANÁLISE COMPLETA →";
  
      $("#cpHomeGames").innerHTML = list.slice(0, 6).map((g, i) => `
        <button type="button" class="cpHomeGame${i === 0 ? " is-first" : ""}" data-v8-game="${i}">
          <time>${g.time}</time>
          <div class="teams"><b>${g.home}</b><i>×</i><b>${g.away}</b></div>
          <small>${g.line}</small><strong>${g.confidence}%</strong>
        </button>`).join("");
  
      $("#cpHomeLastGames").innerHTML = list.slice(1, 4).map((g, i) => `
        <button type="button" class="cpHomeLastGame" data-v8-game="${i + 1}">
          <time>${g.time}</time><b>${g.home}<br>${g.away}</b>
          <strong>${g.confidence}%</strong><i>›</i>
        </button>`).join("");
    }
  
    function chooseMarket(type) {
    ...
    function openMarkets(type = state.activeMarket) {
      ...
      const meta = MARKET[type];
      $("#cpMobileMarketsTitle").textContent = meta.label;
      $("#cpMobileMarketIcon").textContent = meta.icon;
      $("#cpMobileMarketHeading").textContent = `Mercados de ${meta.label.toLowerCase()}`;
      $("#cpMobileOddsGrid").innerHTML = meta.lines.map(line =>
        `<button type="button" data-v8-line="${line}"><b>${line}</b><small>VER JOGOS</small></button>`
      ).join("");
    ...
    function openMatch(game) {
      ...
      const isFinished = /finish|encerr|ft|final/.test(status);
      const isLive = /live|andamento|1h|2h|interval/.test(status);
  
      $("#cpMobileMatchTitle").textContent = isFinished ? "RESUMO PÓS-JOGO" : isLive ? "MATCH CENTER AO VIVO" : "ANÁLISE PRÉ-JOGO";
      $("#cpMobileMatchSubtitle").textContent = isFinished
        ? "Estatísticas finais da partida"
        : isLive ? "Dados e eventos em tempo real" : "Central de inteligência da partida";
  
      $("#cpMobileMatchContent").innerHTML = `
        <section class="cpV8MatchHero">
          <time>${game.time}</time>
          <div><strong>${game.home}</strong><i>×</i><strong>${game.away}</strong></div>
          <span>${game.line}</span>
          <b>${game.confidence}% DE CONFIANÇA</b>
        </section>
        <section class="cpV8MatchTabs">
          <button class="active" type="button">ANTES</button>
          <button type="button">AO VIVO</button>
          <button type="button">DEPOIS</button>
        </section>
        <section class="cpV8MatchStats">
          <div><small>MERCADO</small><strong>${game.line}</strong></div>
          <div><small>CONFIANÇA</small><strong>${game.confidence}%</strong></div>
          <div><small>STATUS</small><strong>${game.status}</strong></div>
          <div><small>HORÁRIO</small><strong>${game.time}</strong></div>
        </section>
        <section class="cpV8MatchRead">
          <h3>LEITURA DA PARTIDA</h3>
          <p>${isFinished ? "Partida encerrada. Consulte os números finais e compare com a projeção pré-jogo." :
            isLive ? "Partida em andamento. Os dados ao vivo serão atualizados pelo Match Center." :
            "Análise baseada no mercado selecionado, projeção e nível de confiança calculado antes da partida."}</p>
        </section>`;
    ...
    function closeLayer(layer) {
      ...
      if (!$("#cpMobileMarketsLayer")?.classList.contains("is-open") &&
          !$("#cpMobileMatchLayer")?.classList.contains("is-open")) {
    ...
    function setupDate() {
      ...
      let input = $("#cpV8DateInput");
      if (!input) {
        input = document.createElement("input");
        input.type = "date";
        input.id = "cpV8DateInput";
        input.style.cssText = "position:fixed;opacity:0;pointer-events:none;left:50%;top:70px;";
      ...
      function paintDate() {
        ...
        $(".cpHomeCalendarMonth").textContent = months[Number(m)-1] || "DATA";
        $(".cpHomeCalendarDay").textContent = d || "--";
      }
      ...
      input.addEventListener("change", async () => {
        ...
        paintDate();
        setLoading(true);
        await loadData().catch(() => setLoading(false));
      });
    }
    ...
    function bind() {
      document.addEventListener("click", e => {
        ...
        if (market) {
          e.preventDefault();
          openMarkets(market.dataset.homeMarket);
          return;
        }
  
        const kind = e.target.closest("[data-cp-market]");
        if (kind) {
          e.preventDefault();
          openMarkets(kind.dataset.cpMarket);
          return;
        }
  
        const line = e.target.closest("[data-v8-line]");
        if (line) {
          e.preventDefault();
          $("#cpMobileSelectedLine").textContent = line.dataset.v8Line;
          $("#cpMobileRecommended").hidden = false;
          $("#cpMobileGameCarousel").innerHTML = activeList().slice(0,5).map((g,i) =>
            `<button type="button" data-v8-game="${i}"><b>${g.home} × ${g.away}</b><small>${g.time}</small><strong>${g.confidence}%</strong></button>`
          ).join("");
          return;
        }
  
        const gameBtn = e.target.closest("[data-v8-game]");
        if (gameBtn) {
          e.preventDefault();
          openMatch(activeList()[Number(gameBtn.dataset.v8Game)]);
          return;
        }
  
        if (e.target.closest("#cpHomeBestOpen") || e.target.closest("#cpHomeMatchOpen")) {
          e.preventDefault();
        ...
        if (close) {
          e.preventDefault();
          closeLayer(close.closest(".cpMobileLayer"));
        }
      }, true);
      ...
      card?.addEventListener("touchend", e => {
        ...
        const keys = ["corners","goals","cards"];
        let i = keys.indexOf(state.activeMarket);
        i = dx < 0 ? (i + 1) % keys.length : (i + keys.length - 1) % keys.length;
        chooseMarket(keys[i]);
      }, {passive:true});
    }
  
    async function start() {
      if (!isMobile() || !$("#cpMobileHome")) return;
      ...
      try {
        ...
        console.error("[Mobile V8]", error);
        setLoading(false);
        $("#cpHomeGames").innerHTML = `<div class="cpHomeEmptyState"><strong>FALHA AO CARREGAR</strong><small>Confira a rota /quentes no servidor.</small></div>`;
      }
    }
