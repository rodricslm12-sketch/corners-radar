/* =========================================================
   ...
   (() => {
    ...
    function openMatch(game) {
      ...
      const isFinished = /finish|encerr|\bft\b|final/.test(status);
      const isLive = /live|andamento|\b1h\b|\b2h\b|interval/.test(status);
  
      const title = $("#cpMobileMatchTitle");
      const subtitle = $("#cpMobileMatchSubtitle");
      if (title) title.textContent = isFinished ? "RESUMO PÓS-JOGO" : isLive ? "MATCH CENTER AO VIVO" : "ANÁLISE PRÉ-JOGO";
      if (subtitle) subtitle.textContent = isFinished
      ...
      if (content) {
        content.innerHTML = `
          <section class="cpV8MatchHero">
            <time>${escapeHtml(game.time)}</time>
            <div><strong>${escapeHtml(game.home)}</strong><i>×</i><strong>${escapeHtml(game.away)}</strong></div>
          ...
          <section class="cpV8MatchStats">
            <div><small>MERCADO</small><strong>${escapeHtml(game.line)}</strong></div>
            <div><small>CONFIANÇA</small><strong>${game.confidence}%</strong></div>
            <div><small>STATUS</small><strong>${escapeHtml(game.status)}</strong></div>
            <div><small>HORÁRIO</small><strong>${escapeHtml(game.time)}</strong></div>
          </section>
          <section class="cpV8MatchRead">
            <h3>LEITURA DA PARTIDA</h3>
            <p>${isFinished
              ? "Partida encerrada. Consulte os números finais e compare com a projeção pré-jogo."
              : isLive
                ? "Partida em andamento. Os dados ao vivo serão atualizados pelo Match Center."
                : "Análise baseada no mercado selecionado, na projeção e no nível de confiança calculado antes da partida."}</p>
          </section>`;
      }
